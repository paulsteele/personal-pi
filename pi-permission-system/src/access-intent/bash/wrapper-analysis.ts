 ...
 * is not flagged.
 */
export function classifyWrapperWords(words: readonly CommandWord[]): WrapperKind | undefined {
 ...
 */
export function executedUnitOf(unitText: string, words: readonly CommandWord[]): string | null {
  let text = unitText;
  let current = words;
 ...
 */
function execTerminatorIndex(words: readonly CommandWord[], start: number): number {
...
function rebase(words: readonly CommandWord[], start: number, end: number): CommandWord[] {
...
const VALUE_TAKING_FLAGS = new Map<string, ReadonlySet<string>>([
  ...
  ["xargs", new Set(["-n", "-P", "-I", "-i", "-d", "-E", "-L", "-l", "-s", "-a"])],
