...
import { EXECUTION_HOST_TYPES, forEachNestedExecution } from "./nested-execution.ts";
...
import {
  ...
  executedUnitOf,
  type WrapperKind,
} from "./wrapper-analysis.ts";

export type { WrapperKind } from "./wrapper-analysis.ts";
...
export interface BashCommand {
  ...
  /** Shell-resolved command words when this unit is a simple command. */
...
export function collectCommands(node: TSNode): BashCommand[] {
  return collectCommandFacts(node).map(({ argv: _argv, ...command }) => command);
}

/** Enumerate command units including argv for deterministic preflight guards. */
export function collectCommandFacts(node: TSNode): BashCommand[] {
...
function makeUnit(
  ...
  const withArgv = argv === undefined ? unit : { ...unit, argv };
  const flagged = wrapperKind ? { ...withArgv, wrapperKind } : withArgv;
 ...
 */
function makeCommandUnit(node: TSNode, context: BashCommandContext | undefined): BashCommand {
  ...
  return makeUnit(
    ...
    words.map((word) => resolveCommandWord(word.text)),
  );
}
...
function resolveCommandWord(text: string): string {
