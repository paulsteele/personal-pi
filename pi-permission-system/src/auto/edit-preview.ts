...
function replacementsFrom(input: Record<string, unknown>): readonly Replacement[] {
  ...
  for (const candidate of candidates) {
    ...
    if (typeof record.oldText !== "string" || typeof record.newText !== "string") continue;
...
export function formatEditForClassifier(input: Record<string, unknown>): string | undefined {
  ...
  const fixedBudget = EDIT_PREVIEW_MARKER.length + (path?.length ?? 0) + shown.length * 90 + 200;
  ...
  if (replacements.length > shown.length) {
    lines.push(`… ${replacements.length - shown.length} additional replacements omitted`);
  }
