...
import permissionSystem from "./pi-permission-system/src/index.ts";
...
export default function localExtensions(pi: ExtensionAPI): void {
	...
	// The integrated local permission/auto fork publishes replayable Activity
	// state before Atelier subscribes; discovery also replays it on reload.
	permissionSystem(pi);
	atelier(pi);
}
